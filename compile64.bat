...
call "%VS120COMNTOOLS%\..\..\VC\vcvarsall.bat" x64 >>"%logFile%" 2>&1
echo.==================================================>>"%logFile%"

echo COMPILING LWNETS && echo.>>"%logFile%" 2>&1

    echo lwnet.sln>>"%logFile%" 2>&1
    echo ./lwnets/lwnet.sln /build Debug
    devenv ./lwnets/lwnet.sln /build Debug >>"%logFile%" 2>&1
...
echo COMPILING DATABASE && echo.>>"%logFile%"
    ...
    echo ./database/database.sln /build Debug
    devenv ./database/database.sln /build Debug >>"%logFile%" 2>&1
...
echo COMPILING AGENTPROC && echo.>>"%logFile%"
    ...
    echo ./agentProc/agentProc.sln /build Debug
    devenv ./agentProc/agentProc.sln /build Debug >>"%logFile%" 2>&1
